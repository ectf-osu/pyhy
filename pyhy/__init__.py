...
def hydro_hash_hash(outlen: int, d: bytes, ctx: bytes, key=ffi.NULL):
    assert len(ctx) == h.hydro_kdf_CONTEXTBYTES
    assert h.hydro_hash_BYTES_MIN <= outlen <= h.hydro_hash_BYTES_MAX
    buf = ffi.new("uint8_t[]", outlen)
    if h.hydro_hash_hash(buf, outlen, d, len(d), ctx, key) == -1:
        return None
    return bytes(buf)
...
def hydro_kdf_derive_from_key(subkey_len, id, ctx, master_key):
    assert (subkey_len >= h.hydro_kdf_BYTES_MIN) and (
        subkey_len <= h.hydro_kdf_BYTES_MAX
    )
    assert (type(ctx) == str) and (len(ctx) == h.hydro_kdf_CONTEXTBYTES)
...
def hydro_secretbox_encrypt(m: bytes, msg_id: int, ctx: bytes, key: bytes):
    assert len(ctx) == h.hydro_kdf_CONTEXTBYTES
    buf = ffi.new("uint8_t[]", len(m) + h.hydro_secretbox_HEADERBYTES)
    h.hydro_secretbox_encrypt(buf, m, len(m), msg_id, ctx, key)
    return bytes(buf)


def hydro_secretbox_decrypt(c: bytes, msg_id: int, ctx: bytes, key: bytes):
    assert len(ctx) == h.hydro_kdf_CONTEXTBYTES
    if len(c) <= h.hydro_secretbox_HEADERBYTES:
        return None
    buf = ffi.new("uint8_t[]", len(c) - h.hydro_secretbox_HEADERBYTES)
    if h.hydro_secretbox_decrypt(buf, c, len(c), msg_id, ctx, key) != 0:
        return None
    return bytes(buf)
...
def hydro_sign_create(m: bytes, ctx: bytes, secret_key: bytes):
    assert len(ctx) == h.hydro_kdf_CONTEXTBYTES
...
def hydro_sign_verify(sig: bytes, m: bytes, ctx: bytes, public_key: bytes):
    assert len(ctx) == h.hydro_kdf_CONTEXTBYTES
    result = h.hydro_sign_verify(sig, m, len(m), ctx, public_key)
    return result == 0
...
def hydro_kx_xx_3(st_client, pkt2, client_kp, psk=None):
    ...
    if (
        h.hydro_kx_xx_3(
            st_client, session_kp_client, pkt3, peer_pk_server, pkt2, psk, client_kp
        )
        != 0
    ):
...
def hydro_pwhash_deterministic(
    pw, ctx, master_key, ops_limit=10000, mem_limit=0, threads=1
):
    if type(pw) == str:
        pw = pw.encode("utf-8")
